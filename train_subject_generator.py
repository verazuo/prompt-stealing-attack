...
def main(args, config):
    ...
    if args.resume:
        ...
        save_dir = os.path.join(args.result_dir, f'pred_subject_resume_{args.dataset}.csv')
    else:
        save_dir = os.path.join(args.result_dir, f'pred_subject_before_train_{args.dataset}.csv')

    model = model.to(device)
    pred_df = evaluate(test_loader, model, config, stop_num=200)
    pred_df.to_csv(save_dir, index=False)
    print("Save pred_df to:", save_dir)
    ...
    for epoch in range(args.start_epoch, config['max_epoch']):
        ...
        pred_df.to_csv(os.path.join(args.result_dir, f'pred_subject_epo{epoch}.csv'), index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='output/PS_subject_generator')    
    ...
    args.output_dir = 'output/PS_subject_generator'
    args.dataset = 'lexica'
