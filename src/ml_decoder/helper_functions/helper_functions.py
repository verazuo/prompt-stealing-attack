...
def save_tensor_img(img, name):
    fig = plt.figure()
    plt.imshow(img.permute(1, 2, 0) )
    fig.savefig(f"{name}.png")

def save_numpy_img(img, name):
    fig = plt.figure()
...
def get_dataset(args):
    ...
    if args.dataset == 'lexica':
        train_dataset = LexicaDataset( return_text="prompt", dataset_dir="vera365/lexica_dataset", mode='train', input_transform=transform_train)
        val_dataset = LexicaDataset( return_text="prompt", dataset_dir="vera365/lexica_dataset", mode='test', input_transform=transform_val)
    else:
