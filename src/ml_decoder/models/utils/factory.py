...
def create_model(args,load_head=False):
    ...
    if args.load_pretrain:
        print("loading pretrain model from {}".format(args.model_path))
        model_path = args.model_path
        if args.model_name == 'tresnet_l' and os.path.exists("./output/pretrained_ckpt/tresnet_l.pth"):
            model_path = "./output/pretrained_ckpt/tresnet_l.pth"
        if model_path:  # make sure to load pretrained model
            if not os.path.exists(model_path):
                print("downloading pretrain model...")
                request.urlretrieve(args.model_path, "./output/pretrained_ckpt/tresnet_l.pth")
                model_path = "./output/pretrained_ckpt/tresnet_l.pth"
                print('done')
